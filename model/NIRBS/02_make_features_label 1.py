...
def compute_spatial_features(df: pd.DataFrame, geo_path: Path) -> pd.DataFrame:
    ...
    for month, month_df in df.groupby("month"):
        ...
        # hardship_index spatial lag (mean hardship of neighboring areas)
        month_full["spatial_lag_hardship"] = libpysal.weights.lag_spatial(w, month_full["hardship_index"].values)
        
        # crime_lag1 spatial lag (mean crime count of neighboring areas in previous month)
...
def main() -> None:
    ...
    # grp = data.groupby("community_area", observed=True)["crime_rate"]
    for lag in [1, 3, 6, 12]:
