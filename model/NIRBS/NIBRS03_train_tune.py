#!/usr/bin/env python3
"""Train and tune LR/RF/XGB on NIBRS train/val splits."""
...
def build_lr(params: Dict[str, object], seed: int) -> LogisticRegression:
...
def _prediction_base_cols(df: pd.DataFrame) -> List[str]:
    cols = []
    for c in ["state_abbr", "month", "target_month", "label"]:
        if c in df.columns:
            cols.append(c)
    return cols
...
def main() -> None:
    ...
    if train_df.empty or val_df.empty:
        raise ValueError("train/val split is empty. Check split configuration in NIBRS02_make_features_label.py.")
    ...
    id_cols = _prediction_base_cols(val_df)
