...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--population", default="population/CCA_20")  
...
def merge_population_and_normalize(model_df: pd.DataFrame, population: str) -> pd.DataFrame:
    model_df['year'] = pd.to_datetime(model_df['month']).dt.year.astype(str)
    pop_df = pd.DataFrame()
    for i in range(15,26):
        pop_path = Path(population + f"{i}.csv")
        if not pop_path.exists():
            raise FileNotFoundError(f"Missing population file: {pop_path}")
        cur_pop = pd.read_csv(pop_path)[['OBJECTID', 'TOT_POP']].rename(columns={'OBJECTID': 'community_area', 'TOT_POP': 'population'})
        cur_pop['community_area'] = cur_pop['community_area'].astype(int)
        cur_pop['year'] = "20" + str(i)
        pop_df = pd.concat([pop_df, cur_pop], ignore_index=True)
    
    df = model_df.merge(pop_df, on=['community_area', 'year'], how='left')
...
def main() -> None:
    ...
    panel = merge_population_and_normalize(panel, args.population)
    panel["crime_rate"] = panel["count_total"] / panel["population"]     # Normalize to crime rate
