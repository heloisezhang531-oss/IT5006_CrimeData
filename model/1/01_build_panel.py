#!/usr/bin/env python3
"""Build community_area-month panel from CSV files or TiDB chicago crime table."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=["csv", "tidb"],
        default="tidb",
        help="Data source for chicago crime records.",
    )
    parser.add_argument(
        "--crime-files",
        nargs="+",
        default=["data/crimes_2015_2024.csv", "data/crimes_2025_2026.csv"],
        help="Input crime CSV files (used when --source=csv).",
    )
    parser.add_argument(
        "--tidb-table",
        default="chicago_crimes",
        help="TiDB source table name (used when --source=tidb).",
    )
    parser.add_argument(
        "--env-file",
        default=".env",
...
def main() -> None:
    ...
    if args.source == "csv":
        ...
        print(f"Fetching monthly aggregates from TiDB table: {table_name}")
