...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input-csv",
        default="",
        help="Optional local CSV with columns event_date,latitude,longitude. If set, TiDB is not used.",
    )
    parser.add_argument(
        "--cache-csv",
        default="",
        help="Optional path to save fetched TiDB points as CSV for offline/cluster jobs.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=400000,
        help="Max training points used for DBSCAN hotspot detection (0 means all).",
    )
    parser.add_argument(
        "--dbscan-backend",
        choices=["auto", "cpu", "gpu"],
        default="auto",
        help="DBSCAN backend. gpu uses cuML DBSCAN when available.",
    )
    parser.add_argument("--output-dir", default="experiment/paper_stcp_reproduction_gpu")
...
def _to_local_xy_meters(df: pd.DataFrame) -> np.ndarray:
    """Project lat/lon to local tangent plane for euclidean clustering."""
...
def detect_crime_dense_regions(
    ...
    if dbscan_backend in {"auto", "gpu"}:
        try:
            import cupy as cp
            from cuml.cluster import DBSCAN as cuDBSCAN

            coords_xy = _to_local_xy_meters(train_df)
            dbscan_gpu = cuDBSCAN(
                eps=float(eps_meters),
                min_samples=int(min_samples),
                metric="euclidean",
            )
            # cuML DBSCAN currently does not support sample weights.
            labels = cp.asnumpy(dbscan_gpu.fit_predict(cp.asarray(coords_xy))).astype(int)
            backend = "gpu-cuml"
        except Exception as exc:
            if dbscan_backend == "gpu":
                raise RuntimeError(f"GPU DBSCAN requested but unavailable: {exc}") from exc
            print(f"[WARN] GPU DBSCAN unavailable, fallback to CPU sklearn: {exc}")
...
def main() -> None:
    ...
    if args.input_csv:
        print(f"Loading crime points from local CSV: {args.input_csv}")
        all_df = pd.read_csv(args.input_csv, parse_dates=["event_date"])
    else:
        ...
        if args.cache_csv:
            ...
            print(f"Cached TiDB points to: {cache_path}")
    if all_df.empty:
        raise RuntimeError("No rows available for the requested date range.")
    required_cols = {"event_date", "latitude", "longitude"}
    missing = required_cols.difference(all_df.columns)
    if missing:
        raise RuntimeError(f"Missing required columns in input data: {sorted(missing)}")
    ...
    all_df = all_df.dropna(subset=["event_date", "latitude", "longitude"]).copy()
    ...
    cluster_train_df = train_df
    if args.cluster_sample_size > 0 and len(train_df) > args.cluster_sample_size:
        cluster_train_df = (
            train_df.sample(n=args.cluster_sample_size, random_state=42)
            .sort_values("event_date")
            .reset_index(drop=True)
        )
        print(
            f"Sampling train points for DBSCAN: {len(cluster_train_df):,} / {len(train_df):,}"
        )

    print("Detecting crime dense regions with weighted DBSCAN...")
    train_cluster_df, eps_rad, dbscan_backend_used = detect_crime_dense_regions(
    ...
    print(f"DBSCAN backend used: {dbscan_backend_used}")
    ...
    summary_lines = [
        ...
        f"- DBSCAN sample size: {args.cluster_sample_size} (0 means all train points)",
        f"- DBSCAN requested backend: {args.dbscan_backend}, used backend: {dbscan_backend_used}",
